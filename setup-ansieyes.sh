...
deploy_to_ec2() {
    ...
    if detect_ec2; then
        ...
        echo "3. Test with: /ansieyes_triage or /ansieyes_prreview (see GUIDE.md for /ansieyes_triage_force)"
...
print_completion_message() {
    ...
    echo
    echo "Documentation:"
    echo "  - Complete Guide: GUIDE.md"
    echo "  - Configuration: triage.config.example.json"
    echo
    echo "Commands:"
    echo "  - /ansieyes_triage        - Issue triage (exact match only)"
    echo "  - /ansieyes_triage_force  - Issue triage, skips prompt-injection check"
